...
def requetetocsv(requete, outfilename):
    cur = con.cursor()
    cur.execute(requete)
...
--     INDIVIDU.COD_PAY_NAT AS code_pays_nationalite,
--     Pays_nationalite.LIB_NAT AS libelle_nationalite,
--     DECODE (SUBSTR (INDIVIDU.COD_PAY_NAT, 1, 1),
--		  '1', 'EUROPE',
--		  '2', 'ASIE',
--		  '3', 'AFRIQUE',
--		  '4', 'AMERIQUE',
...
--	     AS continent,
 ...
 (SELECT count(COD_ETP) from RESULTAT_VET
   WHERE (RESULTAT_VET.COD_ANU = INS_ADM_ANU.COD_ANU)
   AND (RESULTAT_VET.COD_IND = INDIVIDU.COD_IND)
   AND (RESULTAT_VET.TEM_IND_CRN_VET = 'CS')
           AND (RESULTAT_VET.TEM_DST_SUP_VET = 'N')
           AND (RESULTAT_VET.COD_TRE = 'ADM')) as reussite
FROM
      INS_ADM_ETP,
      INS_ADM_ANU,
...
requetetocsv(grande_requete, "UP13.csv")

requete_vdi = """
SELECT COD_IND, COD_ANU, COD_DIP, COD_VRS_VDI, COD_MEN, TEM_DST_SUP_VDI, COD_VRS_VDI, COD_ADM, COD_SES FROM RESULTAT_VDI WHERE
TEM_DLV_AUT_VDI = 'O' AND
COD_ADM = '1' AND
COD_ANU IN
          (
--           '2018',
...
# requetetocsv(reussite_vdi, "reussite_vdi.csv")

reussite = """
SELECT COD_IND, COD_ETP, COD_ANU
FROM RESULTAT_VET WHERE
TEM_IND_CRN_VET = 'CS' AND
TEM_DST_SUP_VET = 'N' AND
COD_TRE = 'ADM' AND
COD_ANU IN
          (
           '2018',
...
--           '2007',
           '2048')"""
# requetetocsv(reussite, "reussite_vet.csv")
